...
quantileFunc<-function(perc, t, statesCurr, y, theta, varInfl, block){
  quants=seq(mean(y)-varInfl/2, mean(y)+varInfl/2, length.out=length(perc))
  unique(quants)
}
...
varInfl=550 #used to set span of quantile approaches
deltaE=0.25 #point in extremal grid cells
