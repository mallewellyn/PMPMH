...
quantileFunc<-function(perc, t, statesCurr, y, theta, varInfl, block){
  if(t<stateLag){
    ...
    m=mean(y)
    quants=pmax(seq(m-varInfl/2, m+varInfl/2, length.out=length(perc)), 0)
    quants=round(quants)
...
q1=0.1 #used to set span of quantile approaches (both R and S)
qN=1-q1 #used to set span of quantile approaches (both R and S)
varInflR=160000 #used to set span of quantile approaches (R)
varInflS=160000 #used to set span of quantile approaches (R)
thresh=0.01
endPropVarR=2 #proposal variance in extremal grid cells (R)
endPropVarS=2 #proposal variance in extremal grid cells (S)
approach=1 #specify approach to setting the grid cells 
