...
e.init<-c(rep(0, state_lag-1), rep(3, length(y)-state_lag+1))
...
log_state_dens<-function(states, t, theta){
  if(t < state_lag){
    ...
    if(t==state_lag){
      p=exp(-theta[1]*epsilon_curr[t+1])
      dens=dpois(states[t], theta[2]*N0*e_curr[t]*exp(-1), log=TRUE) +
        dbinom(S[t+1], S[t]+states[t], p, log=TRUE)
    } else {
      if(t==length(y)){
...
quantile_func<-function(perc, t, states_curr, y, theta, var.infl){
  if(t<state_lag){
    ...
    if(states_curr[t]==0){
      quants=pmax(qpois(perc, var.infl) - var.infl + 1, 0)
    } else {
...
midpoint_int_func_system<-function(mpoints, bin.len, theta, t, state_lag){
  ...
  if(t<state_lag){
    ...
    if(t==state_lag){
      ...
      dens=dens+log(bin.len[[t-state_lag]])
      dens=t(t(dens) + log(bin.len[[t]]))
    }
  }
...
theta_update<-function(states, theta){
  ...
  ## first delta
  log_prop_binom<-function(t, N.curr, delta, epsilon){
    if(t==1){
      dens=-S[1]*delta*epsilon[1] + (N0-S[1])*log(1-exp(-delta*epsilon[1]))
    } else {
      dens=-S[t]*delta*epsilon[t] + (N.curr[t-1]-S[t])*log(1-exp(-delta*epsilon[t]))
    }
    dens
  }
  ...
  if(theta_prop[1]>0){
    pacc[1]=min(1, exp(log_prior_density_delta(theta_prop[1], delta.a, delta.b) +
                         sum(sapply(seq(1, length(y), by=1), log_prop_binom, N.curr=N.curr, delta=theta_prop[1], epsilon=epsilon_curr)) -
                         (log_prior_density_delta(theta_curr[1], delta.a, delta.b) +
                            sum(  sapply(seq(1, length(y), by=1), log_prop_binom, N.curr=N.curr, delta=theta_curr[1], epsilon=epsilon_curr)))))
  }
  if(runif(1)<pacc[1]){
  ...
  if(theta_prop[4]>0){
    pacc[4]=min(1, exp(log_prior_density_beta_e(theta_prop[4], beta.e.a, beta.e.b) +
                         sum(sapply(e_curr[state_lag:length(y)], dgamma, shape=theta_prop[4], rate=theta_prop[4], log=TRUE)) - (
                           log_prior_density_beta_e(theta_curr[4], beta.e.a, beta.e.b) +
                             sum(sapply(e_curr[state_lag:length(y)], dgamma, shape=theta_curr[4], rate=theta_curr[4], log=TRUE)))))
  }
  ...
  if(epsilon_prop[1]>0){
    epsilon_pacc[1]=min(1, exp(dgamma(epsilon_prop[1], theta_curr[3], rate=theta_curr[3], log=TRUE) +
                                 dbinom(S[1], N0, exp(-theta_curr[1]*epsilon_prop[1]), log=TRUE)
                               - (dgamma(epsilon_curr[1], theta_curr[3], rate=theta_curr[3], log=TRUE) +
                                    dbinom(S[1], N0, exp(-theta_curr[1]*epsilon_curr[1]), log=TRUE))))
  }
  ...
  for(t in 2:length(y)){
    ...
    if(epsilon_prop[t]>0){
      epsilon_pacc[t]=min(1, exp(dgamma(epsilon_prop[t], theta_curr[3], rate=theta_curr[3], log=TRUE) +
                                   dbinom(S[t], N.curr[t-1], exp(-theta_curr[1]*epsilon_prop[t]), log=TRUE) - (
                                     dgamma(epsilon_curr[t], theta_curr[3], rate=theta_curr[3], log=TRUE) +
                                       dbinom(S[t], N.curr[t-1], exp(-theta_curr[1]*epsilon_curr[t]), log=TRUE))))
...
end_grid_cell_dens<-function(prop.state, const, end.prop.var){
  dpois(prop.state-const-1, end.prop.var, log=TRUE)
}
mid_grid_cell_dens<-function(prop.state, lower_quant, upper_quant){
  log(1/(upper_quant-lower_quant))
}
start_grid_cell_dens<-function(prop.state, const, end.prop.var){
  log(1/(const+1))
}

#### redefine current indices
find_curr_index<-function(t, states_curr, quants){
  findInterval(states_curr[t], vec=unique(c(-1, quants[[t]], Inf)), left.open = TRUE)
}
################## run algorithm
len.y<-length(y)
...
N.cells<-matrix(rep(0, dim(blocks)[1]*Nits), nrow=Nits)
pacc_theta<-matrix(rep(0, length(theta.init)*Nits), nrow=Nits)
...
timer<-system.time(
  for(i in 2:Nits){
    ...
    #update states
    run_it<-PMPMH_1(states_curr[i-1,], y, blocks, N, q1, qN, var.infl, theta_curr[i-1,], delta.e, thresh, approach, state_lag)
    N.cells[i,]<-run_it$N.cells
