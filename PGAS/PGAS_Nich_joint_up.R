...
PGASJoint<-function(N, rESSMin, y, theta, e, epsilon, RInit, SInit){
  
  particlesR<-matrix(rep(0, N*length(y)), ncol=length(y))
  particlesS<-matrix(rep(0, N*length(y)), ncol=length(y))
  ...
  priorRandInitR<-function(t, theta){
    rpois(1, theta[2]*N0*exp(-1)*e[6])
  }
  priorRandInitS<-function(t, theta){
    rbinom(1, N0, exp(-theta[1]*epsilon[1]))
  }
  
  priorRandOtherwiseR<-function(t, theta, partMinusTauR, partMinusTauS){
    NtMinusTau=partMinusTauS + partMinusTauR
    rand=numeric(length(partMinusTauR))
    for(i in 1:length(partMinusTauR)){
      rand[i]=rpois(1, theta[2]*NtMinusTau[i]*exp(-NtMinusTau[i]/N0)*e[t])
    }
    rand
  }
  priorRandOtherwiseS<-function(t, theta, partMinusS, partMinusR){
    NtMinus=partMinusS + partMinusR
    rand=numeric(length(partMinusS))
    for(i in 1:length(partMinusS)){
      rand[i]=rbinom(1, NtMinus[i], exp(-theta[1]*epsilon[t]))
    }
    rand
  }
  
  logLikelihood<-function(t, theta, partsR, partsS){
    dens=numeric(length(partsR))
    N=partsR+partsS
      for(i in 1:length(partsR)){
  ...
  particlesS[2:N,1]<-sapply(rep(1, N-1), priorRandInitS, theta=theta)
  particlesS[1,1]=SInit[1]
  
  logWeights<-logLikelihood(1, theta, particlesR[,1], particlesS[,1])
  ...
  for(t in 2:tau){
    ...
    particlesS[2:N,t]=priorRandOtherwiseS(t, theta, particlesS[A[-1,t],t-1], particlesR[A[-1,t],t-1])
    particlesS[1,t]=SInit[t]
    
    RInds=seq(t, t+5, by=1)
    RInds=RInds[which(RInds>6)]
    
    RFunc<-function(l, i){
      dpois(RInit[l], theta[2]*e[l]*(particlesR[i,l-tau-1] + particlesS[i,l-tau-1])*exp(-(particlesR[i,l-tau-1] + particlesS[i,l-tau-1])/N0), log=T)
    }
    anFn<-function(i){
      dbinom(SInit[t], particlesR[i,t-1]+particlesS[i,t-1], exp(-theta[1]*epsilon[t]), log=T) +
        sum(sapply(RInds, RFunc, i=i))
    }
    
    aLogWeights=logWeights+ sapply(1:N, anFn)

    
    if(length(which(aLogWeights==-Inf))==N){
      xx<-which(particlesR[,t-1] + particlesS[,t-1]>=SInit[t])
    ...
    particlesS[,1:t]=cbind(particlesS[A[,t],1:(t-1)], particlesS[,t])
   
    logWeights=log(weights[A[,t],t-1]) + logLikelihood(t, theta, particlesR[,t], particlesS[,t])
    if(length(which(logWeights==-Inf))==N){
      logWeights=rep(log(1), N)
    }
  ...
  particlesR[2:N,t]=sapply(2:N, priorRandInitR, theta=theta)
  particlesS[2:N,t]=priorRandOtherwiseS(t, theta, particlesS[A[-1,t],t-1], particlesR[A[-1,t],t-1])
  particlesR[1,t]=RInit[t]
  particlesS[1,t]=SInit[t]
  ...
  asFn<-function(i){
    dbinom(SInit[t], particlesR[i,t-1]+particlesS[i,t-1], exp(-theta[1]*epsilon[t]), log=T) +
      sum(sapply((t+1):(t+5), RFunc, i=i))
      
  }
  aLogWeights=logWeights + sapply(1:N, asFn)

  if(length(which(aLogWeights==-Inf))==N){
    xx<-which(particlesR[,t-1] + particlesS[,t-1]>=SInit[t])
    aLogWeights=rep(log(1), N)
    aLogWeights[xx]=rep(1/length(xx), length(xx))
  }
  const=max(aLogWeights)
  aWeights[,t]<-exp(aLogWeights-const)
  aNormWeights[,t]<-aWeights[,t]/sum(aWeights[,t])
  ...
  particlesR[,1:t]=cbind(particlesR[A[,t],1:(t-1)], particlesR[,t])
  particlesS[,1:t]=cbind(particlesS[A[,t],1:(t-1)], particlesS[,t])
  ...
  for(t in (tau+2):length(y)){
      ...
      particlesR[2:N,t]=priorRandOtherwiseR(t, theta, particlesR[A[-1,t],t-tau-1], particlesS[A[-1,t],t-tau-1])
      particlesS[2:N,t]=priorRandOtherwiseS(t, theta, particlesS[A[-1,t],t-1], particlesR[A[-1,t],t-1])
      ...
      RInds=t:(t+5)
      RInds=RInds[which(RInds<=length(y))]
...
timer<-system.time(
  for(i in 2:its){
   upStates<-PGASJoint(N, rESSMin, y, theta[i-1,], e, epsilon, RSamps[i-1,], SSamps[i-1,])
