...
logLikelihood<-function(t, theta, xt){
...
PGAS<-function(N, rESSMin, y, theta, XInit){
  T=length(y)
  ...
  aWeights<-matrix(rep(0, N*T), ncol=T)
  aNormWeights<-matrix(rep(0, N*T), ncol=T)
  ...
  for(t in 2:T){
    ...
    particles[2:N,t]=sapply(particles[A[-1,t],t-1], priorRandOtherwise, theta=theta)
    particles[1,t]=XInit[t]
    
    ##ancestor sampling
    fixedChild=particles[1,t]
    
    aLogWeights=logWeights + sapply(particles[,t-1], FUN=logPrior, child=fixedChild, theta=theta)
...
timer<-system.time(
  for(i in 2:its){
    #update x samples using PGAS
    xSamps[i,]<-PGAS(N, rESSMin, y, theta[i-1,], xSamps[i-1,])
