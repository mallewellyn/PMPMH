...
PGR<-function(N, rESSMin, y, theta, e, epsilon, RInit, SInit){
...
PGS<-function(N, rESSMin, y, theta, e, epsilon, SInit, RInit){
  ...
  for(t in 2:length(y)){
    ...
    log.weights<-log(weights[A[,t],t-1])+logLikelihood(t, theta, particles[,t])
    if(length(which(logWeights==-Inf))==N){
      logWeights=rep(log(1), N)
    }
...
timer<-system.time(
  for(i in 2:its){
    RSamps[i,]<-PGR(N, rESSMin, y, theta[i-1,], e, epsilon, RSamps[i-1,], SSamps[i-1,])
    SSamps[i,]<-PGS(N, rESSMin, y, theta[i-1,], e, epsilon, SSamps[i-1,], RSamps[i,])
